...
<%@ page import="app.user.vo.UserVO" %>
<%@ page import="java.util.List" %>
<%@ page import="app.products.vo.ProductVO" %>
<%@ page import="java.util.ArrayList" %>
<%@ page import="app.order.vo.CartVO" %>
<%@ page import="app.order.vo.CartProductVO" %>
<!DOCTYPE html>
<html lang="en" >
<head>
    <meta charset="UTF-8">
    ...
    <div id="header">
        <ul>
            ...
            <%
                ...
                if (userVO != null) {
                    out.println("<li><a href=\"user-login.jsp\">LOGOUT</a></li>");
                } else {
                    out.println("<li><a href=\"user-login.jsp\">LOGIN</a></li>");
                    out.println("<li><a href=\"user-signup.jsp\">SIGN UP</a></li>");
                }
            %>
        </ul>
    </div>
...
<%--                    cartVO = (CartVO) request.getSession().getAttribute("cart");--%>
<%--                }--%>

<%--                List<CartProductVO> productVOList = cartVO.getProducts();--%>
<%--                for (CartProductVO cartProductVO: productVOList){--%>
<%--            %>--%>
<%--            out.println("<span>" + cartProductVO.getProduct().getName() + "</span>");--%>
<%--            <%}%>--%>
    ...
    <div id="grid">
        <%
            Object object = request.getSession().getAttribute("products");
            List<ProductVO> products = new ArrayList<>();
            if (object != null) {
                products = (List<ProductVO>) object;
            }
        ...
        <div class="product">
            <div class="info-large">
                <%
                    String h = "<h4>" + productVO.getName() + "</h4>";
                    out.println(h);
                    assert userVO != null;%>
                <div class="sku">
                ...
                <div class="price-big">
                    <span>$43</span> $<%=productVO.getPrice()%>
                </div>
                ...
                <button onclick="handleAddToCart(<%=productVO.getId()%>, <%=userVO.getId()%>, 1)" class="add-cart-large">Add To Cart</button>
            ...
            <div class="make3D">
                <div class="product-front">
                    ...
                    <div class="add_to_cart" onclick="handleAddToCart(<%=productVO.getId()%>, <%=userVO.getId()%>, 1)">Add to cart</div>
...
<script>
    function handleAddToCart(productId, userId, quantity) {
        $.ajax({
            url: 'order',
            method: 'POST',
            data: {
    ...
    // Define a flag to track if the AJAX request has been made
    var ajaxCompleted = false;

    window.onload = function (){
        // if (!ajaxCompleted) {
            $.ajax({
                url: 'home', // Replace with your servlet URL
                method: 'GET',
                dataType: 'text',
                success: function (data) {
                    // Set flag to true to indicate AJAX request completion
                    ajaxCompleted = true;
                    // Optionally, you can process the data received here
                },
                error: function () {
                    // alert('Error occurred while fetching data from the servlet.');
                }
            });
        // }
    }
